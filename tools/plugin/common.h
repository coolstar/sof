...
struct plug_shm_endpoint {
	...
	unsigned long total;		/* total frames copied */
	int frame_size;
...
static inline int plug_ep_get_free(struct plug_shm_endpoint *ep)
{
	...
	else if (ep->rpos == ep->wpos) {
		if (ep->rwrap < ep->wwrap)
			return 0; /* no free frames */
		else
			return ep->buffer_size; /* full buffer free */
	} else
...
static inline int plug_ep_get_avail(struct plug_shm_endpoint *ep)
{
	...
	if (ep->rpos < ep->wpos)
		...
		if (ep->rwrap < ep->wwrap)
			return ep->buffer_size; /* full buffer avail*/
		else
			return 0; /* no avail frames */
	} else
